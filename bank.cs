// Код реализует простую модель банковского счета с возможностью пополнения и снятия денег.
// Используется блокировка с помощью объекта-блокировки balancelock для обеспечения потокобезопасности.
using System;
